<?php 
  ...
  $resultTotalMataPelajaran = mysqli_fetch_array($total_kelas_query);
  $total_kelas = $resultTotalMataPelajaran['total_kelas'];
?>
   ...
   <div class="box">
      ...
      <div class="box-body">
         <div class="row" style="margin-bottom: 10px">
            ...
            <div class="col-md-3 col-md-offset-3">
               <form action="index.php" method="get">
                  <div class="input-group">
                     <input type="hidden" name="page" value="kelola_kehadiran_siswa_sekolah">
            ...
            <div class="col-md-3">
               <form action="index.php?page=kelola_kehadiran_siswa_sekolah&q=<?= isset($_GET['q']) ? $_GET['q'] : '' ?>&halaman=<?= isset($_GET['halaman']) ? $_GET['halaman'] : '' ?>" method="get">
                  <div class="input-group">
                     <input type="hidden" name="page" value="kelola_kehadiran_siswa_sekolah">
                     <input type="hidden" name="q" value="<?= isset($_GET['q']) ? $_GET['q'] : '' ?>">
               ...
               <form action="index.php?page=kelola_kehadiran_siswa_sekolah&q=<?= isset($_GET['q']) ? $_GET['q'] : '' ?>&halaman=<?= isset($_GET['halaman']) ? $_GET['halaman'] : '' ?>" method="get">
                  <div class="input-group">
                     ...
                     <input type="text"  data-toggle="tooltip" data-placement="top" data-original-title="
                        Untuk mencari kelas, masukkan nama kelas, atau jurusan."
         ...
         <table class="table table-bordered ">
            ...
            <?php 
                ...
                $today = date('Y-m-d');
                $cekAbsen = mysqli_query($koneksi, "SELECT * FROM kehadiransiswasekolah WHERE kode_kelas = '$result[kode_kelas]' AND tanggal = '$today'");
                $cekAbsen = mysqli_num_rows($cekAbsen);
            ?>
               <tr class="<?= $cekAbsen == 0 ? 'bg-danger' : 'bg-success' ?>">
                  <td><?= $no++ ?></td>
                  <td><?= $result['tingkat'] ?> <?= $result['jurusan'] ?> <?= $result['kelas'] ?></td>
                  <td><?= $result['jurusan'] ?></td>
                  <td><?= $result['tahun_ajaran'] ?></td>
                  <td>
                        <a href="index.php?page=kelola_kehadiran_siswa_sekolah_lihat&kode_kelas=<?= $result['kode_kelas'] ?>" class="btn btn-info btn-sm"
            ...
            <div class="col-md-12">
               <nav aria-label="Page navigation">
                  ...
                  ">
                     ...
                     ?>
                     <li>
                        <a href="index.php?page=kelola_kehadiran_siswa_sekolah&halaman=<?= $prev ?>&q=<?= isset($_GET['q']) ? $_GET['q'] : '' ?>&jurusan=<?= isset($_GET['jurusan']) ? $_GET['jurusan'] : '' ?>&tahun_ajaran=<?= isset($_GET['tahun_ajaran']) ? $_GET['tahun_ajaran'] : '' ?>"
                     ...
                     <?php for ($i=1; $i <= $total; $i++) { ?>
                      ...
                      ><a href="index.php?page=kelola_kehadiran_siswa_sekolah&halaman=<?= $i ?>&q=<?= isset($_GET['q']) ? $_GET['q'] : '' ?>&jurusan=<?= isset($_GET['jurusan']) ? $_GET['jurusan'] : '' ?>&tahun_ajaran=<?= isset($_GET['tahun_ajaran']) ? $_GET['tahun_ajaran'] : '' ?>"
                     ...
                     ?>
                     <li>
                        <a href="index.php?page=kelola_kehadiran_siswa_sekolah&halaman=<?= $next ?>&q=<?= isset($_GET['q']) ? $_GET['q'] : '' ?>&jurusan=<?= isset($_GET['jurusan']) ? $_GET['jurusan'] : '' ?>&tahun_ajaran=<?= isset($_GET['tahun_ajaran']) ? $_GET['tahun_ajaran'] : '' ?>"
                           data-toggle="tooltip" data-placement="top"
                           title="Halaman <?= $next ?>"
                        aria-label="Next">
